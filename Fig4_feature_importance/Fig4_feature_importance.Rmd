...
for (f in 1:length(featSet)){
  fi_invest = svm_feat_impor(data_WNC_BNC_invest,featSet[f],5,5000)
...
data_WNC_BNC_invest_top14 = data_WNC_BNC_invest %>%
  dplyr::select(dplyr::any_of(as.character(filtered_fi$name)), "good_1") %>%
  dplyr::relocate("good_1", .before = 1)
